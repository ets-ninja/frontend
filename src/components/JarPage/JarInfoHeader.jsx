...
const JarInfoHeader = () => {
  ...
  const { name = 'name' } = useSelector(state => state.basket.basket);
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'row', pr: 2 }}>
      <IconButton
        sx={{ height: 35, width: 35, mt: 0.6, mr: 0.5 }}
        component={Link}
        to="/"
      >
      ...
      <Typography
        ...
        sx={{
          ...
          fontWeight: 700,
          fontSize: 40,
          display: editNameState === false ? 'inline' : 'none',
          flexGrow: 1,
        }}
      >
        {sliceName(name)}
      ...
      <TextField
        sx={{
          display: editNameState === true ? 'inline' : 'none',
          flexGrow: 1,
          heigth: 32,
        }}
        id="outlined-name"
