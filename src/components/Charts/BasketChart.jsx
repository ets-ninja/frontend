...
const BasketChart = () => {
  ...
  const getlabels = () => {
    ...
    oneMonthDateAhead = new Date(
      oneMonthDateAhead.setMonth(oneMonthDateAhead.getMonth() + 2),
    );

    const monthDifference = Math.max(
      Math.floor(
        new Date(oneMonthDateAhead - new Date(creationDate)) / 2629746000,
      ),
      2,
    );
    ...
    for (var i = monthDifference; i > 0; i -= 1) {
      day = new Date(
        oneMonthDateAhead.getFullYear(),
        oneMonthDateAhead.getMonth() - i,
        1,
      );
