...
import StyledMenu from '../UIElements/StyledMenu';
...
const JarDashboardHeader = () => {
  ...
  const { currentType, currentOrder } = useSelector(
    state => state.basket.paginationData,
  );
          ...
          >
            <FormGroup sx={{ marginLeft: 2 }}>
              <FormControlLabel
                control={
                  <Switch
                    checked={archivedBasketsState}
                    onChange={() => {
                      setarchivedBasketsState(!archivedBasketsState);
                    }}
                  />
                }
                label="Archived"
