...
import { changePage } from '../../redux/jar/basketSlice';
import JarCard from '../JarCard';
import { CardSkeleton } from '../publicPage';

const JarDashboard = () => {
  const { baskets, paginationData, loading, error, success } = useSelector(
    state => state.basket,
  );
    ...
    <>
      {error && (
        ...
        >
          <Typography
            variant="h2"
            sx={{ fontFamily: 'Ubuntu', fontWeight: 500, fontSize: 20 }}
          >
            There is an error with loading: {error}
          </Typography>
        </Box>
      )}
      ...
      >
        {!loading && !!baskets?.length && (
          <Pagination
            count={paginationData.maxPageAmount}
